"""Scrape and add 95 Horatio unit from TFC.com"""
import asyncio
...
async def main():
    ...
    url = 'https://tfc.com/west-village-nyc/luxury-no-fee-apartments/west-coast/apt/004h'
    ...
    async with async_playwright() as p:
        ...
        # Check if 95 Horatio building exists
        building = await db.buildings.find_one({'name': '95 Horatio'}, {'_id': 0})
        
        if not building:
            print("\n⚠️ 95 Horatio building not found in database!")
            ...
            print(f"✅ Created 95 Horatio building (ID: {building_id})")
        else:
            building_id = building['id']
            print(f"\n✅ Found 95 Horatio building (ID: {building_id})")
        ...
        unit = {
            ...
            'unit_number': '004H',
            'rent': rent if rent > 0 else 4500.0,  # Use detected or default
            'bedrooms': bedrooms,
            'bathrooms': bathrooms,
            'square_feet': None,
            ...
            'description': 'Luxury apartment in West Village',
        ...
        print(f"✅ Created unit 004H with {len(unit['images'])} images!")
        
        print(f"\n📊 Unit Details:")
        print(f"   - Unit Number: 004H")
        print(f"   - Rent: ${unit['rent']}")
        print(f"   - Bedrooms: {unit['bedrooms']}")
        print(f"   - Bathrooms: {unit['bathrooms']}")
        print(f"   - Building: 95 Horatio")
        print(f"   - Location: West Village, New York")
