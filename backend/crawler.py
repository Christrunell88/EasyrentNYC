...
async def crawl_building_to_staging(building_id: str, batch_id: Optional[str] = None):
    ...
    # Use modular scraper (now includes all scrapers including generic fallback)
...
async def crawl_new_building_to_staging(
    ...
    elif 'melar' in source_url or 'themelar' in source_url:
        units_data = await crawl_melar(source_url)
    else:
