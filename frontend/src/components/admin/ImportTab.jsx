...
import { Building2, Plus, RefreshCw, Download, ExternalLink, Sparkles } from 'lucide-react';
...
const ImportTab = ({ fetchStagingStats }) => {
  ...
  const handlePropertyCrawl = async (property) => {
    ...
    try {
      ...
      // Note: Import preview dialog is handled in parent via crawledData state
      // For now, we auto-import
      if (response.data.building && response.data.units) {
        try {
          const importRes = await axios.post(`${API}/admin/property-import`,
            { building: response.data.building, units: response.data.units },
            { withCredentials: true }
          );
          ...
          toast.error('Import failed after crawl.');
        }
      }
    } catch (error) {
